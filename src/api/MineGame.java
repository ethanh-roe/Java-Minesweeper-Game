...
import static api.CellType.FLAGGED;
	...
	public void endGame() {
		...
		for (Cell[] element : grid) {
			for (int c = 0; c < grid[0].length; c++) {
				if (element[c].getType().equals(MINE)) {
					element[c].setUncoved();
				}
			}
		}
		MenuPanel.endGame();
	...
	public void revealNearbyEmptyCells(int row, int col) {
		...
		if (!c.isCovered()) {
			...
			if (c.getType().equals(BLANK) && !c.isFlagged()) {
				...
				for (BoardLocation neighbor : neighbors) {

					if ((neighbor.x > -1 && neighbor.x < rows)
							&& (neighbor.y > -1 && neighbor.y < cols)) {

						c = grid[neighbor.getRow()][neighbor.getCol()];

						if (c.getType().equals(BLANK) && c.isCovered()) {

							revealNearbyEmptyCells(neighbor.getRow(), neighbor.getCol());
